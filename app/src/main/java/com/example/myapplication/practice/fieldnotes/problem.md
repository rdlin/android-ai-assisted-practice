...
Starter code is already included in the project. It renders seeded notes from an in-memory repository and has a route wired through the practice launcher. Your task is to iterate on that baseline, not start from a blank app.

Focus on a single-device offline experience. You do not need a real backend.

## Starter Code Available

- Route: `PracticeRoute.FieldNotes`
